...
export const PackingSlipDialog = ({ open, onClose, transferData }: PackingSlipDialogProps) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    // Fix: The property should be 'contentRef' not 'content'
    contentRef: printRef,
  });
        ...
        {!showCourierForm ? (
          <>
            ...
            <DialogFooter className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row sm:space-x-2">
              ...
              <Button onClick={generatePDF} variant="default" className="gap-2 w-full sm:w-auto">
                <Printer className="h-4 w-4" />
                Save PDF
              </Button>
              <Button onClick={handleShowCourierForm} variant="default" className="gap-2 w-full sm:w-auto">
